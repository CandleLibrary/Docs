...
import { TSNodeTypeLU, parser, renderWithFormatting } from "@candlefw/ts";
...
const fsp = fs.promises, library = process.cwd(), fn_regex = /(file\:\/\/)(\/)*([A-Z]\:)*/g, doc_dir = path.join("/", import.meta.url.replace(fn_regex, ""), "../../.."), cfw_font = txt_to_svg.loadSync(path.join(doc_dir, './source/font/cfw.otf'));
function mergeComments(ast, comments) {
    for (const comment of comments) {
        let closest_node = null;
        if (!closest_node || closest_node.pos.off < comment.off) {
            for (const { node } of traverse(ast, "nodes")) {
                if (node.pos) {
                    if (node.pos.off < comment.off)
                        continue;
...
function renderDocumentSection(name) {
    return `[Docs](https://cfw.acweathersby.com/${name})`;
}
function renderPollyFillBrief(ast) {
    return `### TODO - FILL THIS IN! \n\`\`\`ts\n${renderWithFormatting(ast)}\n\`\`\``;
}
function renderCandleFWDepends(name, cfw_depends) {
    return `
...
function renderHeaderAndLogo(name, package_json, library_root) {
    const logo_url = URL.resolveRelative("./source/brand/logo.svg", library_root), source_icon_url = URL.resolveRelative("./source/brand/go_to_source.svg", library_root), source_icon = fs.readFileSync(path.join(doc_dir, "./source/svg/go_to_source.svg"), { encoding: "utf8" }), logo_partial = fs.readFileSync(path.join(doc_dir, "./source/svg/cfw.partial.svg"), { encoding: "utf8" }), name_path = cfw_font.getPath(name.toUpperCase(), { x: 0, y: 0, fontSize: 32, anchor: 'top', attributes: { fill: "rgb(50,50,50)" } }), cfw_path = cfw_font.getPath("CFW", { x: 0, y: 0, fontSize: 14, anchor: 'top', attributes: { fill: "rgb(100,100,100)" } }), logo = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
...
async function gatherFragmentDocuments(p) {
    const file_names = [];
    try {
...
function sanitizeSourceURL(file_url) {
    return file_url.toString().replace(process.cwd(), "");
}
async function attachFragmentsToSections(sections) {
    for (const file_url of await gatherFragmentDocuments(path.join(doc_dir, "source/doc_fragments/"))) {
        try {
            const text = await file_url.fetchText(), text_lex = new Lexer(text);
            text_lex.source = sanitizeSourceURL(file_url);
            const { value, error } = lrParse(text_lex, fragment_parser_data, null);
            if (!error)
                for (const doc_fragment of value) {
                    const start = doc_fragment.pos.copy(), end = doc_fragment.pos.getEnd();
                    while (!end.END)
                        end.next();
...
function attachCommentsToSections(comments, sections) {
    for (const comment of comments) {
        const { value, error } = lrParse(comment.slice(), comment_parser_data), end = comment.getEnd();
        if (!error) {
            addFragmentToSections(value, sections, `${comment.source}`, `L${comment.line + 1}-L${end.line + 1}`);
        }
    }
}
function addFragmentToSections(doc_fragment, sections, origin = "undefined", line) {
...
(async function () {
    await URL.server();
    const library_root = new URL(library + "/"), package_json = await URL.resolveRelative("./package.json", library_root)
        .fetchJSON()
        .catch(e => {
        ...
        process.exit(1);
    }), package_root_file = URL.resolveRelative(package_json
        .main
        .replace("./build/library/", "./source/typescript/")
        .replace(".js", ".ts"), library_root), 
    //Grab the package of the library
    readme_url = URL.resolveRelative("./README.md", library_root), cfw_depends = Object.entries((package_json.dependencies ?? {}))
        .map(([key, val]) => key)
        .filter(d => d.includes("@candlefw"))
        .map(e => e.replace("@candlefw/", "")), name = package_json.name.replace("@candlefw/", ""), root_file = await package_root_file.fetchText(), 
    //Document Sections -----------------------------------------------
    ...
    fs.writeFileSync(readme_url.toString(), Object.entries(sections).sort(([a], [b]) => {
        const a_index = section_header_indices[a.toLowerCase()] ?? Infinity, b_index = section_header_indices[b.toLowerCase()] ?? Infinity;
        if (a_index == b_index)
            return 0;
        return a_index < b_index ? -1 : 1;
    }).filter(([, n]) => n.length > 0).map(([key, nodes]) => `${key.toLowerCase() == "header" ? "" :
        "# " + key.replace(/\_/g, " ").toUpperCase()}\n${nodes.join("\n")}`).join("\n\n"));
})();
//# sourceMappingURL=build_readme.js.map